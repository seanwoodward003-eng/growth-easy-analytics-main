...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Extract host from URL params (required for App Bridge in iframe)
  ...
  // Loading fallback if host not ready (prevents blank app)
  if (!host) {
    return (
      <html lang="en">
        <head>
        ...
        <body className={`${orbitron.className} bg-[#0a0f2c] text-cyan-200 min-h-dvh`}>
          <div className="flex items-center justify-center h-screen">
            <p className="text-cyan-300 text-xl">Loading app...</p>
          </div>
        </body>
      </html>
    );
    ...
    <html lang="en">
      ...
      <body className={`${orbitron.className} bg-[#0a0f2c] text-cyan-200 min-h-dvh relative overflow-x-hidden`}>
        <AppBridgeProvider config={{ apiKey: process.env.NEXT_PUBLIC_SHOPIFY_API_KEY || '', host }}>
          ...
          {/* Your custom side menu – kept as mobile fallback */}
          {menuOpen && (
            <>
