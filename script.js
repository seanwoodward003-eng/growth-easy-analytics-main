/*====================================================================
  ...
  - Cookie auth only (no manual token)
  - Connect buttons open popups and refresh on close
  - No more loops or login_required
====================================================================*/
...
async function apiFetch(endpoint, options = {}) {
  try {
    const res = await fetch(`${BACKEND_URL}/${endpoint}`, {
      ...
      credentials: 'include'  // ← Cookie does all the work
    });
...
function renderDashboard(data, isFake = false) {
  ...
  // Draw chart (keep your drawChart function if you have it)
  if (window.drawChart) {
    const history = (data?.revenue?.history || FAKE.revenue.history);
    drawChart(history.labels, history.values, data?.revenue?.trend || '+6%');
  }
}

// Connect provider – FIXED POPUP THAT WORKS EVERY TIME
window.connectProvider = (provider) => {
  ...
  if (provider === 'shopify') {
    ...
    if (!shop || !shop.includes('.')) return alert('Invalid store');
  ...
  const check = setInterval(() => {
    if (popup.closed) {
      clearInterval(check);
      refreshData();  // Refresh dashboard when popup closes
    }
  }, 500);
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mobile menu (your original – unchanged)
